...
export async function esUpdateDoc(
  FIREBASE_DB,
...
export async function esAddUser(
  FIREBASE_DB,
  userCredential,
  collectionName: string,
  data
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const docRef = await setDoc(
        doc(FIREBASE_DB, collectionName, userCredential.user.uid),
        { id: userCredential.user.uid, uid: userCredential.user.uid, ...data }
      );
      resolve({
        error: false,
        docRef,
...
export async function esSetDoc(
  FIREBASE_DB,
  collectionName: string,
  docId: string,
  data: object
...
export async function esDeleteDoc(
  FIREBASE_DB,
  collectionName: string,
  docId: string
) {
...
export async function esDeleteDocs(
  FIREBASE_DB,
  collectionName: string,
  collectionPropertyValue: string,
  targetId: string
) {
  return new Promise(async (resolve, reject) => {
    try {
