...
export function userLanguagesMatchesExchange(
  exchange: ExchangeFormatted,
  user: User
) {
  const learningLanguageValues = Object.values(
    exchange.learningLanguageUnfolded
  );
  const teachingLanguageValues = Object.values(
    exchange.teachingLanguageUnfolded
  );
...
export function checkUserIsValidToJoin(
  ...
  // Already Joined
    ...
    message = joiningUserIsMe
      ? "You have already joined this exchange"
      : `User ${joiningUser.username} has already joined this exchange`;
  }
  // // Is organizer
  ...
  // exchange full TL
    ...
    participantsTeachingLanguage[0].teachingLanguageId ===
      joiningUser.teachingLanguageId
  ) {
  ...
  // exchange full LL
    ...
    participantsLearningLanguage[0].learningLanguageId ===
      joiningUser.learningLanguageId
  ) {
...
export async function removeMyselfFromExchange(FIREBASE_DB, me, exchange) {
  ...
  await esUpdateDoc(FIREBASE_DB, "exchanges", exchange.id, {
    participantIds: participantsMeRemoved,
  });
  return {
    success: true,
...
export function getPartipantsOfLanguages(users, exchange) {
  ...
  const participantsTeachingLanguage = users.filter(
    (user) =>
      exchange.participantIds.includes(user.id) &&
      user.teachingLanguageId === exchange.teachingLanguageId
  );
  const participantsLearningLanguage = users.filter(
    (user) =>
      exchange.participantIds.includes(user.id) &&
      user.learningLanguageId === exchange.teachingLanguageId
  );
...
export async function addParticipantToExchange(FIREBASE_DB, exchange, user) {
  ...
  const { error, response } = await esUpdateDoc(
    FIREBASE_DB,
    "exchanges",
    exchange.id,
    { participantIds: participantsUserAdded }
  );
